...
class SHA512(HASH):
    ...
    def extension(self, 
        ...
        init_block = [
            int.from_bytes(signature[i : i + 8], byteorder='big') for i in range(0, len(signature), 8)
        ]
    ...
    def __hashing(self, init_block=None, last_blocks=None):
        ...
        # Resulting 512-bit message digest
        return  (h0).to_bytes(8, byteorder='big') + (h1).to_bytes(8, byteorder='big') + \
                ...
                (h6).to_bytes(8, byteorder='big') + (h7).to_bytes(8, byteorder='big')